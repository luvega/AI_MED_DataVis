...
本章按“真实案例负责展示完整问题，本地小对象负责复现基本操作”的方式组织。人骨髓案例来自 sc-best-practices 已保存的运行结果，本机没有重新连接远程 LaminDB 对象；Seurat 案例则在本机从头运行。两类证据在正文中分别标明。

| 材料 | 本章用途 | 使用边界 |
| --- | --- | --- |
| sc-best-practices `site4-donor8` 人骨髓数据 | 贯穿质控、归一化、特征选择、PCA、UMAP、Leiden、marker 和 CellTypist | 复述素材已有代码、数值与图，不声称本机重跑远程对象 |
| OSCA 10x PBMC 工作流 | 对照 empty droplet、线粒体比例、scran、方差建模、SNN 聚类与 marker 解释 | 案例参数只适用于其数据与分析目的 |
| Seurat `pbmc_small` | 本机演示对象检查、归一化、200 个高变特征、PCA、邻接图、聚类、UMAP 和 marker 图 | 仅 80 个细胞、230 个特征，无可用线粒体基因，不承担完整质控或医学结论 |
| AIDD 单细胞教学材料 | 核对 Seurat 教学顺序和初学者操作负担 | 仅采用可与其他材料相互印证的流程，不沿用其中不稳定术语 |
| 项目术语表、图表规范和课程大纲 | 统一术语、图注字段、学习目标与作业证据 | 不替代生物学证据和方法学原始记录 |
