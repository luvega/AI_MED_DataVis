...
5. 按问题、数据、方法、结果和边界审阅一个教材案例。
6. 为一个小型案例建立问题说明、数据字典、运行记录、图表设计规范卡、解释卡和 AI 协作记录。

对应产物包括全书路线说明、来源与观察单位卡、任务类型标记、解释卡，以及包含 AI 建议、人工修改和核验依据的协作记录。

## 阅读导航

本书所说的证据，是一组能够从结论回查到数据来源、处理步骤和解释条件的记录。单个图形或模型分数不足以形成这条证据链。程序得到数值后，还要说明输入文件、处理规则、参数和解释范围。

五篇内容共用一条主线：先定义问题和数据对象，再检查质量与方法，随后形成图表或模型结果，最后核对解释边界和人工责任。

| 篇章 | 主要问题 | 典型对象 | 主要产物 |
| --- | --- | --- | --- |
| 第一篇：课程入口、工具链与 AI 协作规范 | 问题怎样定义，项目怎样组织，AI 怎样受约束 | 规则文件、任务说明书、案例字段表 | 项目目录、任务说明书、AI 协作记录 |
| 第二篇：数据结构、读取、清洗与探索性分析 | 原始记录怎样变成可分析数据 | 列表、数据框、长表、宽表 | 数据字典、清洗记录、探索性汇总 |
| 第三篇：图表表达、统计推断与基础建模 | 差异、关联和预测怎样规范表达 | 分析表、模型矩阵、预测结果 | 图表设计规范卡、统计报告、模型评估表 |
| 第四篇：高维医药数据与 bulk 组学分析 | 大量特征、公共数据和组学流程怎样核验 | 高维矩阵、计数矩阵、元数据、序列文件 | 参数卡、差异结果表、下载与引用记录 |
| 第五篇：单细胞、空间组学与综合项目 | 细胞、样本、空间和多模态对象怎样整合 | 单细胞对象、空间坐标、邻接图、多模态对象 | 综合报告、复现包与图表审阅记录 |

阅读案例时先确认数据对象和观察单位，再判断任务属于描述、比较、关联、预测还是无监督探索。结果文字应区分观察、计算、统计推断与医学解释；缺少研究设计、独立样本或外部证据时，写明仍需验证的内容。
...
![图1-1 从药学问题到可核验证据包的五个环节](assets/chapter-1-analysis-evidence-journey-imagegen.png)

*图1-1　五个环节依次表示：1，提出药学问题；2，确认数据对象；3，执行分析并设置检查点；4，形成图表和结果表；5，由人核对并整理证据包。本图由 ImageGen 生成，用于说明对象关系，不作为事实或结果证据。步骤含义以相邻流程图和正文为准，使用时仍需人工核对。*

## 核心内容

### 1.1 医药数据处理与可视化的课程定位

一项医药数据任务通常同时要求四种判断。分析者要先用药学知识界定对象和结局，再把问题落实到可处理的数据；随后选择适合的定量方法和图表；最后说明结果的来源、限制与责任归属。编程贯穿其中，却只负责把已经说明的规则执行出来。对象或问题定义错误时，代码运行成功也只会稳定地产生错误答案。

这四种判断会在后续章节反复出现。第一章先把它们放进同一条工作链，使读者在看到软件、函数或模型名称之前，能够先说清任务究竟要回答什么，以及完成后凭什么判断结果可用。
...
问题确定以后，下一步是查明数据从哪里来、怎样形成以及用什么结构保存。来源决定记录能够代表什么，形态决定程序可以怎样读取。两者不能相互替代。一个结构规整的表格仍可能缺少采集背景；一份来源清楚的数据也可能需要先解析成表、矩阵或序列对象，才能进入分析。

因此，来源检查应当先于方法选择。分析者要把产生记录的场景、观察单位、文件结构和使用权限连在一起，随后才讨论缺失值、统计比较或可视化。这样做能减少把不同层级数据放进同一分析口径的风险。
...
该案例使用 PCA 和层次聚类探索表达结构。拟合无监督方法时不使用癌症类型标签，分析完成后再比较聚类或低维位置与已有标签的对应情况。教学目的在于说明大量特征怎样构成高维矩阵，以及无监督结果怎样接受外部标签复核；当前材料不用于预测患者癌症。
...
这个案例先检查数据链：原始 reads 怎样形成基因计数，计数矩阵的列怎样与元数据的行对齐，4 个细胞系的配对结构怎样进入后续设计。第一章只认识这些对象，差异基因筛选、标准化和差异表达留到第12章讨论。
...
识别数据来源时，不要停在“这是一个 CSV”或“这是公共数据”。CSV 只说明文件格式，公共数据只说明取得渠道。分析还取决于数据为何产生、谁被记录、字段如何定义、版本是否固定，以及当前是否有权使用。
...
数据被读入后，文件中的行列要重新对应到研究对象。这一步需要确认每个术语在当前任务中具体指向什么，给字段贴上名称还不足以完成判断。表中一行可以是一名患者、一次访视、一个生物样本或一笔交易；矩阵的一列也可能代表样本、细胞或测量条件。只有先确定观察单位，样本量、分组和统计独立性才有明确含义。

变量的角色同样随问题而变。同一字段在一个任务中可能用于分组，在另一个任务中可能只是调整因素。结局、解释变量和分组变量需要在分析前确定，并与 ID、批次、时间点和重复测量一起写入数据说明。后续清洗、作图和建模都依赖这一步。
...
医学或药学问题往往先以自然语言出现，其中可能同时包含人群、处理、时间、结局和因果期待。数据分析问题需要把这些成分拆开，确认当前数据实际包含哪些对象和变量，再选择描述、比较、关联、预测或无监督探索中的一种主要任务。任务类型确定后，方法和产物才有选择依据。

转换过程还要保留不能回答的部分。若数据只有横断面关联，就不能把任务目标写成因果效应；若只有细胞实验结果，就不能把输出预设为临床疗效判断。把边界写进问题说明书，可以在代码生成和结果解释之前阻止任务越界。
...
分析过程只有留下可检查的痕迹，读者才能判断结论怎样形成。数据字典说明字段含义，清洗记录说明数据怎样变化，脚本和运行结果说明方法怎样执行，图表设计规范卡说明视觉编码，解释卡则把观察、方法和允许解释分开。任何一份文件都不能单独替代其余环节。

课程项目把这些材料组织成相互指向的证据包。图表应能回到作图数据和脚本，脚本应能回到输入文件与处理规则，文字结论应能回到具体输出。AI 协作记录补充谁提出了什么建议、哪些内容经过人工修改和核验，但不能代替真实运行结果。
...
AI 记录要说明人在何处做了判断，并给出相应的核验依据。单独声明“使用过 AI”不能完成这项记录。工具生成的内容越多，验证记录越需要具体。
