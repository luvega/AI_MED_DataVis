...
读本章时先看“表的一行”，再看函数。`melt()`、`merge()`、`groupby()`、`hist()` 执行具体操作，结果能否解释还取决于观察单位、主键、分母、单位和数据来源。
...
![图6-1 从记录表到分析就绪数据的主要变换](assets/chapter-6-table-transformation-imagegen.png)

*图6-1　五个环节依次表示：1，确认一行代表什么；2，根据任务进行长表与宽表转换；3，按主键连接并核对未匹配记录；4，分组汇总，同时保留分布检查；5，输出分析表、变换记录和核验结果。本图由 ImageGen 生成，不提供真实数据结论。转换规则和行数变化以正文、代码和实际输出为准，使用时仍需人工核对。*
...
数据整形从确认一行代表什么开始。医药数据的一行可以对应样本或交易，也可以记录一次随访、一个检测项目、某个药品、基因或时间点。长宽转换会改变行列布局，却不应改变原始观察的数量和含义；表连接则可能增加、丢失或重复记录，因此更需要明确主键。

运行转换代码前先写出当前观察单位、目标观察单位和唯一键，预测行数与列数怎样变化。运行后检查键的唯一性、未匹配记录、重复组合和汇总值。若连接后的行数超出预期，先查一对多或多对多关系，不直接把新增行当作更多样本。
...
转换前，宽表有 4 行，每行是一个样本。转换后，长表有 12 行，每行是“一个样本的一个检测指标”。行数增加来自观察单位的改变，不能据此判定转换错误。
...
分组汇总在预先定义的组内计算 n、均值、中位数、比例或其他描述统计量。结果是否可读，首先取决于分组字段、观察单位和缺失处理是否正确。若同一供体的重复测量被当作独立行，或缺失值在各组处理方式不同，汇总表会给出整齐但含义错误的数字。

代码运行前说明分组变量、统计对象、分母和缺失规则；运行后核对各组 n、总数和手工抽查结果。描述统计回答样本数据大致呈现什么状态，不判断差异是否能推广到总体。统计推断将在第8章结合独立单位、效应量和不确定性继续处理。
...
描述统计表会压缩分布信息。两个组的均值可能接近，但一个组集中、另一个组波动较大；均值也可能被少数极端观察拉动。直方图、箱线图和分位数帮助查看位置、离散程度、偏态和候选异常点，使汇总数字重新对应到观测分布。

异常候选需要回到记录层检查。代码给出的 IQR 标记只表示数值位于规则范围之外，不说明该值错误，更不能自动决定删除。下一步要核对单位、录入、样本状态和研究设计，并记录保留或处理理由。若极端值真实存在，分析还应检查结论对该观察的敏感性。
...
探索性图表用于正式推断或建模前查看变量分布、分组模式、异常观察、时间变化和数据质量。图形选择由问题和变量结构共同决定。单变量分布、组间描述、时间变化和两变量关系需要不同编码，不能因为某种图形熟悉就重复使用。

作图前先固定数据范围、观察单位、横纵轴和分组编码；出图后核对点数、组数、单位和图例，再描述可直接观察的模式。图中分离、趋势或极端点可以提出下一步检查，却不能单独代替统计设计或医学证据。后续分析应由图中暴露的问题决定，而非从图形外观倒推预设结论。
...
日期、分类变量和文本字段决定记录能否正确排序、分组和检索。日期若仍是普通字符串，月份顺序和时间间隔可能错误；分类标签中的大小写、空格或同义写法会制造假组别；文本字段清理过度则可能删除药品名称中的有效信息。这些问题通常在汇总后才暴露，回查成本较高。

处理前先保存原始字段并列出转换规则。日期解析后检查失败值和时间范围，分类标准化后比较转换前后的唯一值，文本清理后保留原值与新值对照。每种转换都应回答“哪些记录改变了、是否符合规则、失败项怎样处理”，再把稳定字段交给分组汇总或作图。
