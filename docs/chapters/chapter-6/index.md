...
本章主线是“分析表 -> 观察单位 -> 长宽表转换/表连接 -> 分组汇总 -> 描述统计 -> 探索性图表 -> 解释边界”。学生要能说明一张表的一行代表什么，也要能说明每个图表回答什么问题。
...
临床指标示例用于训练描述统计表，不用于医学判断。`ALT` 和 `AST` 的单位、参考范围、采样时间和检测方法都需要数据字典或教师说明。
...
分布检查主要回答三个问题：数值大多落在哪里，波动有多大，是否存在需要回查的异常观察。这个阶段的目标是发现问题，不是写医学结论。
...
未匹配的分类水平不能凭感觉归类。上例中的 `unknown` 应写入需人工确认清单。真实作业中，分类合并规则要来自数据字典、教师说明或数据提供者。
...
```text
目标：
请为《医药数据处理与可视化》第6章“数据整形、描述统计与探索性可视化”生成一张教学知识结构图。

上下文：
学生是药学本科生和研究生，不默认熟悉高级统计、生信或编程。
图中必须包含：观察单位、长表、宽表、主键、表连接、分组汇总、描述统计量、直方图、箱线图、异常观察、探索性图表、日期、分类变量、文本字段、解释边界、AI 协作记录。

约束：
不加入 P 值、置信区间、效应量、医学因果、临床建议或未给出的样本量。
节点文字要短，关系要清楚。
...
教师提供一张临床指标宽表和一张样本分组表。学生需要把指标宽表转成长表，并按 `sample_id` 连接分组表。
...
教师提供一张模拟药品销售表或简化临床指标表。学生需要生成描述统计表、异常观察清单和两张探索性图表。
...
| 位置 | 缺口 | 处理方式 |
| --- | --- | --- |
| 药品销售案例 | 仓库未发现原始 `drugSale.csv` 或 `cleanedData.csv` | 正文使用内嵌摘录行，不写实测结论 |
| `6536` 行结果 | 来自教材运行示例 | 标为教材摘录 |
| 销售数量单位 | 素材未说明盒、瓶、件或其他 | 数据字典写 `需人工确认` |
| 应收/实收规则 | 折扣、医保支付、退款规则未说明 | 只做逻辑标记 |
| 药品名称标准化 | 通用名、商品名和别名关系未提供 | 只列候选，需人工确认 |
| 组学示例 | 本章只用简化矩阵和元数据 | 不写差异表达或生物学机制 |

## 统一课程融合接口

本章增加airway样本级质量案例。学生从`raw_counts.csv`按样本求和得到library size（文库大小，即一个样本进入计数矩阵的总计数），再与metadata中的处理和细胞系连接。连接只用于检查结构和质量，不进入差异表达。

| 检查 | 计算或整理 | AI容易给出的错误建议 | 人工判断 |
| --- | --- | --- | --- |
| 文库大小 | 每个样本计数列求和 | 自动删除最小文库 | 结合预设规则、技术记录和整体分布判断 |
| 分组平衡 | `cell_line × treatment`交叉表 | 只比较总组数 | 检查4个细胞系是否各有一对样本 |
| 样本错配 | metadata顺序与counts列名逐项比较 | 集合相同即通过 | 集合和顺序都要核对 |
| 异常观察 | 分布图和分组汇总 | 把离群点直接改为缺失 | 先标记，再查来源和影响 |

保存的QC表至少包含`observation`、`possible_cause`、`decision`和`evidence`四列。AI可以提出候选原因，不能替教师或学生作删除决定。
