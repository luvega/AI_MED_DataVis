...
| 表连接需要主键和行数核验 | 组学 metadata、表达矩阵、药品编码示例 | 可写成课堂检查规则 | 不让 AI 猜连接键含义 |
| 描述统计回答数据概貌 | ISL `describe()`、药品销售汇总材料 | 可写成入门流程 | 不写 P 值或组间差异结论 |
| 图表用于探索和质控 | ISL 直方图/箱线图、教材销售图示例 | 可写成图表选择建议 | 不写医学因果或临床建议 |
| 日期、分类和文本要先标准化 | 药品名称、购药日期、metadata 分组字段 | 可写成清洗要求 | 文本解释需人工确认 |
...
第7章会系统讲图表契约。本章使用简版版本，让学生先学会把图表问题写清楚。
...
日销售趋势图的输入应是日汇总表，而不是原始交易明细。下面代码只生成图表源数据和绘图对象，示例结果来自内嵌教材摘录行。
...
图中必须包含：观察单位、长表、宽表、主键、表连接、分组汇总、描述统计量、直方图、箱线图、异常观察、探索性图表、日期、分类变量、文本字段、解释边界、AI协作记录。
...
| 组学示例 | 本章只用简化矩阵和 metadata | 不写差异表达或生物学机制 |
