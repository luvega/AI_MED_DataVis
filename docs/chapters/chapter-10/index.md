...
| 位置 | 当前缺口 | 正文处理 |
| --- | --- | --- |
| 正式课堂数据 | 使用本章固定教学模拟数据 | 明确标注模拟属性，不作为真实医学证据 |
...
本章把AI辅助建模固定为一个可审计闭环。AI可以在每个环节生成草稿，但前一环节未验收时不得自动进入下一环节。
...
学生要在代码中证明标准化、缺失填补和特征选择只在训练数据上拟合，再应用到验证或测试数据。若AI先在全数据选择特征，即使最终AUC较高，也属于信息泄漏。8样本airway不进入该建模闭环；它用于高维结构和差异结果复核，而不是制造一个看似准确的预测器。

统一交付包括任务定义、数据划分清单、pipeline、指标表、校准或限制说明、泄漏审计、环境和AI日志。内部评价不能替代外部验证，特征重要性也不能单独证明生物学机制。
