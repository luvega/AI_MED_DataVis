...
本节是独立的数据结构案例，不是前述分类模型的延伸。它只讲时间、事件、删失、Kaplan-Meier（KM）估计和风险比（hazard ratio, HR）的读表边界。
...
模型可解释性（interpretability）说明输入变量怎样影响模型输出。系数、树的切分规则、变量重要性和局部解释图属于模型证据；生物系统解释需要额外研究证据。
