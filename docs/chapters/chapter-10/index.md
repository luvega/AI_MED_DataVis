...
AI 可生成局部 Python 或 R 代码，并协助整理指标表和报错。数据划分、泄漏检查、正类与事件定义、特征处理和医学解释由学习者核验。
...
模型评价统一按“拆分→fold 内预处理→建模→评价→解释→报告”推进。数据填补、标准化、特征选择和调参都在训练数据或各训练 fold 内完成；保留测试集只用于既定流程的最终评价。
...
训练集用于拟合模型参数，也用于学习均值、标准差、缺失填补规则和特征筛选规则。验证集参与模型或参数选择，测试集只在流程确定后使用，用于估计整套流程在未见记录上的表现。数据划分保护的是评价独立性，不只是把行随机分成几个文件。

任何从全数据学习的信息都可能跨越划分边界。先在全表筛特征、填补缺失或标准化，再切分训练与测试，会让测试数据间接影响模型。若同一患者、供体或批次的相关记录被分到两侧，也可能产生实体级泄漏。预处理和拆分方式因此要与真实预测单位一起记录。
...
重采样（resampling）从已有数据反复构造训练和验证子集，用来观察结论对样本变化的敏感程度。交叉验证主要用于估计测试误差或选择模型复杂度；bootstrap 常用于估计统计量、参数或模型指标的波动。两者都受原始样本代表性和独立结构限制。

选择重采样方案时，折数或重复次数并非唯一问题。分层、分组、时间顺序和嵌套调参决定验证子集是否模拟真实使用场景。结果应报告中心趋势和波动，不能只挑最好的一次切分。若不同划分下性能变化很大，稳定性本身就是需要解释的结果。
...
特征（feature）是模型的输入变量。特征选择可能用于减少噪声、降低计算量、改善泛化或简化模型；正则化则通过约束系数控制复杂度。两类方法都在训练流程内部学习，必须在每个训练折中重新执行，避免测试或验证信息提前进入特征集合。

筛出的变量只说明它们在当前数据、候选范围、模型和验证设计下有助于预测或拟合。变量被保留、系数不为零或选择频率较高，都不能自动解释为生物学机制。报告还应展示选择稳定性、相关特征的替代关系和未纳入变量的范围。
...
`lambda.min` 取交叉验证指标最优处，`lambda.1se` 取距离最优值一个标准误范围内、惩罚更强的候选。二者对应不同的性能与简洁性取舍。
...
决策树（decision tree）用一系列切分规则形成预测路径。浅树便于查看规则，但单棵树对训练数据变化敏感。bagging、随机森林和 boosting 通过组合多棵树改善预测稳定性，同时也使最终决策不再由一条简单路径概括。

变量重要性衡量特征在特定模型和数据中的贡献方式。分裂增益、置换重要性或其他指标的口径不同，变量相关性也会改变排名。重要性适合解释模型依赖哪些输入，不提供变量对结局的因果效应。稳定性检查应在重采样或独立数据中重复计算。
...
生存分析面对“从起点到事件”的结局，时间和事件状态必须同时进入数据对象。只把是否发生事件当作二分类结局，会丢失随访时长和删失信息。这个独立的数据结构案例只讲时间、事件、删失、Kaplan-Meier（KM）估计和风险比（hazard ratio, HR）的读表边界。
...
模型可解释性（interpretability）说明输入变量怎样影响模型输出。系数、树的切分规则、变量重要性和局部解释图分别提供不同视角，它们的含义依赖模型、背景数据和计算方法。解释材料首先属于模型证据，不能直接描述生物系统中的因果作用。

报告顺序应从任务和数据开始，再写验证设计、整体性能、校准或阈值表现，最后呈现全局与局部解释。解释结果还要经过稳定性检查，并与可能的替代模型比较。若需要提出生物学或临床含义，必须另行提供研究设计和外部证据。
...
AI 可以协助检查字段、生成代码骨架和整理报告，但每个数字都要回到运行结果，每个解释都要经过主张、证据与边界核对。
...
```mermaid
flowchart LR
  R["第10章"] --> N1["10.1 训练集、测试集与数据泄漏"]
  N1["10.1 训练集、测试集与数据泄漏"] --> N2["10.2 交叉验证与 bootstrap"]
  N2["10.2 交叉验证与 bootstrap"] --> N3["10.3 特征选择与正则化"]
  N3["10.3 特征选择与正则化"] --> N4["10.4 树模型与变量重要性"]
  N4["10.4 树模型与变量重要性"] --> N5["10.5 生存分析入门"]
  N5["10.5 生存分析入门"] --> N6["10.6 模型可解释性与报告规范"]
```
