...
相关分析先从散点图读取两个变量的共同变化。点云的方向、形状、范围、分组和异常点决定相关系数是否适合作为概括。若关系明显弯曲、由少数点驱动或只在某个亚组出现，单个系数会掩盖重要结构。

因此，计算前先确认观察单位和变量尺度，画出带原始点的散点图；计算后把系数、置信信息或 P 值放回图中检查。相关描述的是当前数据中的共同变化，不提供时间顺序，也不能排除共同原因和选择偏差。
...
线性回归把连续结局与一个或多个自变量的关系写成可估计模型。模型系数的含义取决于公式、单位和同时纳入的变量。增加一个协变量后，其他系数可能变化；这种变化首先反映模型条件改变，不能直接解释为发现了新的生物学过程。

拟合只是分析的中点。结果产生后要检查残差形态、极端与高影响观察、预测范围和模型假设，再决定系数是否适合报告。诊断图暴露的是模型与数据之间的不一致，处理方式需要结合测量、设计和分析目标判断。
...
分类模型面对离散结局，首先要定义正类、预测单位和使用场景。逻辑回归输出的是在当前模型与输入下属于正类的预测概率。这个概率需要经过数据拆分、校准和外部验证等检查，才能讨论在新数据上的表现。

逻辑回归沿用公式建模的思路，但结局尺度已经改变。系数描述自变量与 log odds 的模型关系，概率则由全部输入共同计算。把概率转换成类别还需要阈值，阈值选择会影响错误类型，却不会改变原始模型系数。
...
扩展表保留相同字段，用确定性公式生成 200 个样本，其中 `response=1` 有 62 个。确定性规则让 Python 和 R 得到同一数据，便于复算阈值和分类指标；这份模拟表不代表真实疾病分布。
...
阈值把连续预测概率变成分类决定，也把错误分成假阳性和假阴性。混淆矩阵记录这四类结果，灵敏度和特异度从不同条件人群出发计算。阅读任何指标前，都要先确认正类定义、矩阵方向和分母。

ROC 曲线展示阈值变化时灵敏度与假阳性率之间的关系，AUC 概括模型区分正负类的能力。它们不说明概率是否校准，也不会自动给出适合实际应用的阈值。若不同错误的后果不同，阈值还需结合任务风险和验证数据选择。
...
模型报告需要分开描述数据、拟合关系、预测表现和决策后果。训练数据中的统计关联不能代替未见数据上的性能，较高的 AUC 也不能代替概率校准、亚组稳定性或临床净获益。每一层都应注明对应的证据和仍未完成的验证。

进入医学决策讨论前，还要明确预测结果将影响谁、何时使用以及错误会造成什么后果。教材案例只训练风险识别和报告结构，不据此提出诊疗建议。若缺少外部验证、真实患病率或决策阈值依据，应用结论应停留在候选模型或教学演示层面。
