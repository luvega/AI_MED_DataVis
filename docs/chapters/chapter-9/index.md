...
本章使用同一组字段构造两张教学模拟表。24 行基础表可以逐行查看，适合手算、相关和线性回归。200 行扩展表按确定性规则生成，用于逻辑回归、阈值和 ROC。两张表都不含真实患者或真实药物记录。
...
统计显著不等于实际意义，AUC 较高也不等于概率准确或临床有用。模型报告需要把已完成的分析与尚未完成的验证分开写。
...
```text
请生成一张中文教学知识图谱，用于《医药数据处理与可视化》第9章
“相关、回归与分类模型”。中心节点为“从数据关系到基础模型报告”。
...
- 残差诊断连接线性回归与模型边界
- 预测概率通过阈值转为类别
- 混淆矩阵连接 TP、FP、TN、FN 与五项分类指标
- AUC 只表示当前数据范围内的区分能力
- 相关不等于因果，模型预测不等于临床决策

图中文字使用短标签。不要加入真实患者、具体诊疗阈值、药物机制或临床建议。
```

若使用 imagegen 生成教学图，图片只作展示。文字、节点或连线若不清楚，以本节 Mermaid、表格和正文为准，并在图注中标明“需人工核验”。
...
| 位置 | 当前缺口 | 本章处理 |
| --- | --- | --- |
| 正式课堂数据 | 使用本章既有确定性医药教学模拟表 | 明确标注模拟属性，不作为真实医学证据 |
| ALT/AST 医学解释 | 无适用人群、检测条件和临床阈值 | 只作为连续教学变量 |
| response 含义 | 无真实结局定义 | 只写模拟阳性/阴性 |
| 医学阈值 | 无任务后果、指南或专业共识 | 不指定最佳阈值 |
| 泛化能力 | 本章未划分训练/测试集 | 所有指标标明在拟合数据上计算 |
| 外部验证与校准 | 未提供外部数据 | 留到后续模型评价和真实研究 |
| 高维特征 | 本章不做特征选择 | 用 `9_Tumors` 说明边界，方法留到第10章 |

## 统一课程融合接口

airway只有8个bulk样本，且4个细胞系构成配对结构。本章明确不把它用于相关、回归或分类预测教学，否则极易把探索性拟合误写成可泛化模型。课程已有医药表格小案例继续承担模型训练，但使用与组学章节相同的AI核验流程。

| 环节 | AI可协助 | 学生必须核验 |
| --- | --- | --- |
| 任务定义 | 整理结局、解释变量和预期图表 | 变量含义、时间顺序和使用场景 |
| 代码生成 | 生成局部相关、回归或逻辑回归代码 | 数据类型、缺失处理和样本数变化 |
| 诊断 | 建议残差图、异常点和共线性检查 | 假设是否满足及不满足的后果 |
| 解释 | 起草模型摘要 | 相关不等于因果，拟合数据表现不等于泛化 |

模型卡必须写出数据范围、训练或拟合范围、观察结果、替代解释和未完成验证。任何临床阈值、诊断价值或用药建议均需额外证据。
