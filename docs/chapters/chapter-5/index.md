...
本章进入全书第二阶段“提示词编程（Prompt Coding）”。第4章已经训练学生识别对象、列名、索引和缺失值，本章把这些能力放到真实文件现场。学生要学习如何把 CSV、TXT 或 Excel 安全读入，再用数据字典和质量检查判断下一步能不能分析。
...
教师提供一个模拟药品销售表、元数据（metadata）表或简化组学字段表。学生需要完成读取、数据字典、质量检查和清洗记录，不要求做统计推断。
...
统一教学数据包让本章从“单个CSV能否读入”推进到“四张表能否可靠连接”。连接前先检查集合，连接时显式重排，连接后再核对行数和缺失。两个文件含有相同样本ID，并不说明当前顺序一致。

| 文件 | 主键或连接键 | 首轮检查 | 静默错误风险 |
| --- | --- | --- | --- |
| `sample_metadata.csv` | `sample_id` | 8行、唯一、处理与细胞系配对 | 标签错位或样本重复 |
| `raw_counts.csv` | `gene_id`；列名为sample_id | 63,677个源基因、8个样本列、非负计数 | 样本列顺序与metadata不一致 |
| `gene_annotation.csv` | `gene_id` | Ensembl ID唯一性、symbol缺失 | 一对多映射被强行压成一行 |
| 文件清单 | 相对路径 | 大小、状态与SHA-256 | 同名旧文件被误读 |

AI可以根据真实列名起草数据字典，但`biological_unit`、`treatment`和基因ID版本必须回查`provenance.md`。学生需保存集合差、重排索引和连接前后行数，不能让AI自行删除未匹配ID或覆盖`raw_counts.csv`。
