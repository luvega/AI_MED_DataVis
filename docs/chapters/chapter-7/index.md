...
科研图表统一按问题、数据、视觉编码、图注、导出和人工核验六项检查。该顺序同样适用于基础统计图、PCA、热图、火山图、UMAP 和空间图。
...
一张科研图应先有可观察的问题，再有与问题匹配的数据和视觉编码。若问题是查看分布，图中需要保留个体值或分布形状；若问题是比较组别，分组、样本量和不确定性就要能够被识别。先选图形再寻找数据，容易让熟悉的模板替代真正的分析目标。

数据来源限定图中每个点、柱或色块能够代表什么。视觉编码只把变量映射到位置、颜色、形状或大小，不能补足缺失的研究设计。开始作图前，应把图表问题、观察单位、纳入规则和主要编码写在同一张规范卡中；出图后再逐项核对。
...
表7-1汇总当前纳入记录，所有数量均为未加权记录数，不代表美国成人总体估计。
...
坐标轴、图例、颜色和标注共同规定读图方式。轴的范围与变换影响差异看起来有多大，图例说明颜色或形状对应哪个变量，标注帮助定位关键对象，多子图则要求不同面板之间保持可比较的尺度和编码。任一元素含义不清，真实数据也可能被错误呈现。

下面先审阅一张故意设置问题的图，再逐项修正。审阅时先确认单位、零点或变换和分组映射，再看标题与注释是否超出数据能够支持的范围。装饰性调整放在这些检查之后，不能用配色或版式掩盖样本量、缺失和不确定性。
...
图7-1　NHANES ALT 问题图。数据值来自真实公开记录，标题、纵轴范围和信息缺口则是为审阅练习设置的错误表达，不应作为分析结论。
...
图注把图中的视觉对象重新连接到数据和方法。读者不查看正文时，也应能从图注知道每个面板展示什么、颜色和符号怎样编码、样本如何纳入、误差线或统计符号代表什么。图注缺少这些信息时，图形往往只能提供印象，不能独立复核。

统计信息应与图中实际呈现相匹配。若图显示描述性分布，就报告观察数和汇总口径；若加入检验结果，则说明比较对象、方法、多重校正和不确定性。图注只报告已经由输出支持的结果，不把视觉趋势扩写成因果、疗效或机制判断。
...
可复现图表由作图数据、脚本、参数、图注和导出文件共同组成。作图数据说明哪些记录进入图，脚本落实转换和编码，参数固定统计或尺度选择，图注解释读图规则，导出文件则保存最终呈现。只保留图片会丢失生成过程，只保留脚本也无法证明使用了哪一版输入。

复核时按这条依赖链逆向检查。先确认最终图片和图注一致，再回查视觉编码与脚本，最后核对作图数据、纳入规则和来源许可。R 与 Python 只需保持数据语义、坐标和分组一致，不要求像素完全一致。任何输入或规则改变后，都应判断图表是否需要重新生成。

| 核验项 | 最低记录 |
| --- | --- |
| 数据 | 来源、纳入规则、缺失、分组和每图有效 n |
| 参数 | 转换、统计量、阈值、随机种子和软件版本 |
| 图形 | 坐标、单位、尺度、颜色、形状、子图和标签 |
| 来源 | 图注、引用、许可或重绘说明 |
...
这种分级也适用于同伴审阅。先指出会改变结论的问题，再处理美观。反馈需要写明对象和修改理由，例如“子图 b 的 AST 有效样本数应单独报告”，避免只写“图不好看”。
...
```mermaid
flowchart LR
  R["第7章"] --> N1["7.1 图表问题、数据来源与视觉编码"]
  N1["7.1 图表问题、数据来源与视觉编码"] --> N2["7.2 坐标轴、图例、颜色、标注与多子图"]
  N2["7.2 坐标轴、图例、颜色、标注与多子图"] --> N3["7.3 按问题和变量选择医药数据图表"]
  N3["7.3 按问题和变量选择医药数据图表"] --> N4["7.4 图注写作与统计信息呈现"]
  N4["7.4 图注写作与统计信息呈现"] --> N5["7.5 作图数据表、脚本与可复现导出"]
```
