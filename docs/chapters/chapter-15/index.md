...
多样本单细胞分析先要把 cell、sample、donor、batch 和 condition 放进同一层级表。cell 属于 sample，sample 可能属于 donor，batch 记录技术来源，condition 表示研究比较。若 batch 与 condition 完全重合，任何校正都无法仅凭数据分辨技术差异和条件差异。

数据整合主要改善不同样本或批次在共享表示中的可比性，条件比较则需要在合适的独立单位上估计差异。整合后的 UMAP 混合良好，只能说明所选表示中的批次结构减弱；它不能证明生物差异完整保留。比较前还应回查原始样本组成和未整合表达层。
...
轨迹、RNA velocity 和细胞通讯分析都从横截面单细胞数据构造模型化线索，但它们估计的对象不同。轨迹给出表达状态的相对顺序，velocity 利用 spliced 与 unspliced RNA 关系估计局部方向，通讯分析将细胞群表达与配体和受体知识组合成候选相互作用。

三类方法都依赖上游质控、特征、邻接关系和细胞注释。根细胞、动力学假设或数据库选择变化时，结果可能改变。报告应把计算输出写成排序、方向估计或候选关系，并列出可能的批次、状态连续性与先验数据库偏差；真实时间、谱系和功能作用仍需额外验证。
...
空间组学对象在表达矩阵之外增加坐标、组织图像和空间邻接关系。空间单位可能是 spot、bin、细胞、细胞核或分子，同一坐标点包含的生物对象随平台和分割方法而变。开始分析前必须确认一行表达记录对应哪种空间单位，以及它属于哪张切片、哪个 sample 和哪个 donor。

空间图首先展示位置与信号的共同分布。相邻 spot 或细胞可以形成空间富集和邻域候选，但邻近本身不等于细胞通讯或组织形成机制。跨切片或跨供体比较还需处理坐标不可直接重合、组织区域差异和独立重复问题。
...
多模态对象把同一细胞或同一样本的不同测量连接起来。CITE-seq 结合 RNA 与 ADT，免疫受体数据增加 TCR 或 BCR 链信息，multiome 可同时测量 RNA 与染色质可及性。整合前应先确认模态是否真正来自同一 cell barcode，以及过滤和匹配后还保留哪些 sample 与 donor。

每种模态有独立的噪声、缺失和尺度，联合表示不能消除这些差异。对齐后要检查未匹配条目、模态质量、批次和条件分布，再解释共享或互补结构。跨模态一致性可以增强某项观察的可复核性，却不能自动升级为因果或机制证据。
...
综合项目从一个可由现有数据回答的问题开始，再确定分析单位、输入对象、方法链、停止条件和交付物。选题过宽时，算法会不断增加，证据责任反而变得模糊。一个合适的问题应能指出主要比较或关系，也能明确当前数据不负责回答什么。

分析路线要把中间对象交代清楚。原始或预处理输入经过哪些检查，哪些参数产生结果表和图，哪些输出支持文字判断，都应在项目说明书中对应。交付物不是彼此独立的文件清单，报告中的每项主张都要能够回到代码、数据、图表和核验记录。
...
分析前先输出对象摘要。本对象没有 `sample_id` 列，单切片标识 `V1_Adult_Mouse_Brain` 位于 `.uns["spatial"]`。这一状态属于对象的实际结构，AI 不应把它当成缺失值补全。若要比较条件，须另外引入多张切片及样本元数据。
...
项目审阅沿证据链逐层收口。先查来源、许可和文件身份，再核对 cell、sample、donor、batch、condition 等对象层级；随后检查代码路径、参数和运行状态，最后比较结果表、图表与汇报结论。前一层无法确认时，后面的解释需要暂停或降级。

汇报应区分实际运行结果、素材中已有结果和教学简化内容。代码能够执行只证明当前环境通过了相应步骤，图表清晰也只证明表达形式可读。项目是否可复核取决于输入、变更、输出和主张能否相互对应，并由提交者说明仍未解决的问题。
...
进阶单细胞和空间组学的共同难点是对象、设计和解释能否对齐，增加图形数量不能解决这些问题。批次整合要同时检查技术混合与生物保留；条件推断要回到 sample 或 donor；轨迹、velocity 和通讯结果要保留模型假设；空间分析要记录坐标与邻接；多模态分析要先核对 barcode 和缺失。
