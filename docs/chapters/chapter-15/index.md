...
第14章已经介绍单细胞表达矩阵、质控、归一化、降维、聚类和细胞类型注释。本章转向更复杂的分析审阅：多个样本能否合并，条件差异是否被批次校正抹去，轨迹和箭头能解释到什么程度，空间坐标怎样进入分析，多种模态如何按细胞条形码对齐，以及一项综合项目怎样形成可复核的交付物。
...
本章案例使用三种证据状态。`本机复跑`表示代码在本项目独立环境中执行并产生输出；`素材已运行`表示来源 notebook 保存了代码与输出，但本轮没有在本机重跑；`教学简化`表示数据为课程构造，只用于解释结构或检查步骤。
...
以下信息用于定位素材结果，不表示本机复跑。单元编号按当前 notebook 的 `cells` 数组从 0 计数；文件变动后，应先重算哈希再引用单元编号。
...
| 案例 | notebook 与关键单元 | SHA-256 | 可获得的来源环境 |
| --- | --- | --- | --- |
...
**证据状态：本机复跑。** 本轮从 scverse 示例数据服务取得 `visium_hne_adata.h5ad`。文件经字节数、SHA-256 和 HDF5 格式核验后读入。对象含 2,688 个 spot、18,078 个基因和 15 个 `cluster`，`obsm["spatial"]` 为 2,688×2 坐标，`uns["spatial"]` 只含一张切片 `V1_Adult_Mouse_Brain`。
...
**证据状态：本机复跑。** 本轮通过 SCBP 的 LaminDB 键 `surface-protein/cite_filtered.h5mu` 取得 MuData。缓存对象为 1,544,072,864 字节，SHA-256 为 `ba4d002df7c9c38f29427e1a5aed361516a769384972cbad8873a68c6fbd081f`。对象含 122,016 个 droplet、36,601 个 RNA 基因和 140 种 ADT，供者为 `s1d1` 至 `s4d9` 的 12 个原始 ID。
...
该素材位于 `44_multimodal_integration_paired_integration/notebook.ipynb`，条形码交集与 MuData 构建对应单元 16、20，文件 SHA-256 为 `60F9FA7E3E1260952706E6C987DBB7119F8A1105E718A30FF77E347EA1D7909D`。notebook 未保存完整运行环境，相关版本标记`需补证据`；因此本节只复用对象对齐逻辑和来源输出，不声称重现方法排名。
...
| 数据来源 | scverse `visium_hne_adata.h5ad`，本地文件 SHA-256 为 `3571e6b8…56e1cbe` |
...
### 15.6.1 八页汇报模板：用空间邻域项目填充

汇报要让听众能追溯数据、代码和结论。以下八页已经填入 15.5 项目的实际内容。每页保留一个主要问题，页脚标注文件位置和证据状态。

| 页码 | 本项目内容 | 现场要回答的问题 |
| --- | --- | --- |
| 1 | 题目：真实 Visium 空间邻域的复现与参数审阅 | 分析对象和核心问题是什么 |
...
同伴复核不直接改写原文件。复核者先记录问题、证据和建议，作者再逐项回应。AIDD 的 GitHub 协作材料可支持分支、提交、Pull Request、Issue 和冲突处理的课程训练；科学判断仍由项目作者和教师确认。
...
| 维度 | 权重 | 达标证据 |
| --- | ---: | --- |
| 问题与数据来源 | 15% | 问题可回答，来源、许可和证据状态清楚 |
| 数据对象与质量 | 15% | 数据字典、元数据、对齐和数量记录完整 |
| 分析路线 | 20% | 方法服务问题，统计单位和参数合理 |
| 代码与复现 | 20% | 脚本化、环境固定、路径可移植、错误有记录 |
| 图表与图注 | 15% | 规范卡完整，图形可读，作图数据表可追踪 |
| 解释与限制 | 10% | 区分观察、方法、推断和验证缺口 |
| AI 协作记录 | 5% | 提示词、输出、人工修改和测试结果齐全 |

高分项目不以方法数量取胜。一个只完成空间邻域分析、但对象清楚、代码可运行、图表可复核且结论克制的项目，优于堆叠多种算法却无法说明输入和统计单位的项目。
...
每个方法节点连接四类信息：输入对象、关键参数、可视化输出、解释边界。
单独标出 sample/donor 统计单位、barcode 对齐、空间邻接规则和证据状态。
不要把拟时序、velocity、配体受体、空间邻近或克隆型画成医学因果。
若生成图片，节点文字和关系必须再用 Mermaid 或表格人工核验。
```
...
综合项目把这些要求变成可评分证据：数据字典说明对象，脚本和环境说明计算，图表设计规范卡说明表达，研究解释卡说明边界，AI 协作记录说明人工判断。项目结论可以有限，但证据链必须清楚。

## 统一课程融合接口

课程收束到一个统一项目，不设置表格与NGS两条轨道。默认项目使用airway数据包完成问题定义、来源核验、质量检查、高维分析、差异结果和证据审阅；更换数据集时仍须提交同构产物和同一证据链。

| 证据 | 可观察或计算的量 | 允许支持 | 不能单独支持 |
| --- | --- | --- | --- |
| bulk RNA | 样本层表达与差异 | 条件相关表达变化 | 蛋白变化、直接调控、临床疗效 |
| ATAC peak | 区域开放性信号 | 染色质可及性线索 | 转录因子真实结合或功能后果 |
| motif | 序列模式匹配 | 候选结合可能性 | 真实结合和调控方向 |
| ChIP信号 | 特定蛋白相关区域富集 | 实验条件下的结合相关证据 | 下游表达因果或患者结局 |
| 单细胞 | 细胞群与表达模式 | 细胞类型或状态线索 | 供体层因果和天然标签真值 |
| 空间结果 | 坐标上的表达或邻域模式 | 空间共定位线索 | 细胞间因果通讯 |

不同样本或不同研究的结果只能并列为证据，不得拼接成同一个已证实机制。AI可以汇总证据卡，但不得把“共同出现”改写为“直接调控”。

统一项目按40%评分：问题与来源8%，处理与分析10%，图表与图注8%，解释与证据边界8%，复现交付和AI披露6%。过程性考核仍为60%。答辩时学生必须用数据、代码和记录回答来源、统计选择、图表和医学边界问题。
