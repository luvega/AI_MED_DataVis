...
第12章从已有计数矩阵出发，训练输入核对、实验设计、统计输出阅读和边界化解释。比对算法推导与生产级命令不在课程范围内。
...
RNA-seq 的上游流程不断改变数据对象。FASTQ 保存 reads 与碱基质量，质控报告概括测序状态，比对或准比对步骤把 reads 与参考序列联系起来，定量再把片段汇总为基因或转录本计数。每一步的输出既是下一步的输入，也是发现上游问题的检查点。

本课程使用预计算计数矩阵，不在课堂运行完整比对与定量流程。读者仍要知道计数来自哪一版参考和注释，样本文件如何映射到矩阵列，以及哪些质控结论不能从计数矩阵反推。上游记录缺失时，后续差异分析只能在已提供矩阵的范围内解释。
...
差异表达需要两个同步输入。计数矩阵提供每个基因在各样本中的计数，metadata 提供样本的处理、细胞系、批次或其他设计因素。矩阵列名与 metadata 行名必须逐一对应，顺序也要一致；相似的名称不能替代显式核对。

对象对齐后，再把研究问题写成设计和比较方向。airway 案例的处理状态与细胞系共同进入分析，contrast 明确 treated 相对 untreated。若忽略细胞系的配对结构，组内变异的解释会改变；若交换参照水平，log2FoldChange 的符号也会反向。
...
原始计数同时反映生物表达、测序深度和文库组成，不能把不同样本中的数值直接当作可比浓度。差异表达模型通过 size factor 等方式降低技术尺度差异，再在计数分布模型中估计处理效应。标准化服务模型比较，不会消除批次、错配或不合理实验设计。

展示用途通常使用另一种转换矩阵，使均值与方差关系更适合 PCA、距离或热图。模型输入、标准化计数和展示矩阵应分别命名并保存，避免把转换值送入要求原始计数的方法，也避免把展示色块当作绝对表达量。
...
差异表达结果表把比较方向、效应估计、不确定性和多重检验证据放在同一行。阅读时先确认基因 ID 与 contrast，再看 `log2FoldChange` 的方向和大小，随后检查标准误、统计量、P 值和 `padj`。只按颜色或阈值挑选条目，会丢失模型条件与估计精度。

结果表中的每个基因都来自同一设计下的统计比较，但并不具有相同的表达水平、数据质量或估计稳定性。低计数、缺失的调整后 P 值和极端效应都需要结合相应字段检查。筛选规则用于形成展示或后续核验集合，不把条目自动升级为机制证据。
...
按本轮版本、预过滤、模型及 `padj < 0.05`、`|log2FoldChange| >= 1` 规则，999 个基因进入展示集合，其中 519 个在 treated 方向较高，480 个较低。更换任一条件都可能改变该数量。下表为完整结果按 `padj` 排序后的前 6 行，数值来自本地运行，表中基因只用于学习结果字段和排序规则。
...
差异分析中的图表分别回答不同问题。PCA 检查展示矩阵中的样本主要变化，火山图把各基因的效应估计与统计证据放在同一平面，热图则展示选定基因在样本间的相对模式。三类图使用的对象和筛选规则不同，不能用一张图替代另一张图的证据。

读图顺序从来源对象开始。先确认变换矩阵或结果表，再核对样本、基因选择、尺度和颜色，最后描述直接可见的模式。图中分离、密集点云或色块一致性可以支持数据层面的观察，却不单独证明处理机制、样本分型或质量合格。
...
功能富集（functional enrichment）检查候选基因或排序统计量是否集中于预定义功能类别、通路或基因集。输入列表、背景集、ID 映射、数据库版本和检验类型共同决定结果。分析运行成功只说明这些对象被方法处理，不能证明对应通路在样本中已经被直接测量或激活。

富集表需要与差异表达结果保持可追溯关系。先记录候选集或排序统计量怎样产生，再核对未匹配 ID、背景范围和多重校正，最后查看条目冗余与驱动基因。解释应停留在当前基因集合与数据库注释的一致性；机制或功能结论仍需独立实验与文献证据。
...
课程数据包已按上调、下调方向分别生成 GO Biological Process ORA 结果，GSEA 未运行。ORA 使用 `org.Hs.eg.db 3.23.1` 与 `GO.db 3.23.1`，以 13,927 个具有有效映射且进入检验范围的基因为背景；上调集合 391 个基因，下调集合 376 个基因。超几何检验后在各方向内进行 BH 校正，共保存 4,242 条结果。结果表位于 `outputs/2026-08-21-NGS00移交包归档与课程融合更新/教学数据包/enrichment_results.csv`，2026-08-24 的独立复核重新计算了五个示例条目和全部方向内 BH 值。

这些结果支持“所选差异基因集合相对背景在相应 GO 注释集合中统计过度代表”。注释冗余、阈值、背景集和映射规则都可能改变结果；ORA 不能证明通路被激活、基因间存在直接调控或结果具有临床作用。下面的表格用于审阅这条解释边界。
...
- 核验标准：对象层级正确；参考与注释版本匹配；未把流程跑通写成数据可靠。

### 作业2：airway 设计与结果复现

- 任务：运行本章 R 代码，提交元数据对齐检查、`~ cell + dex` 设计矩阵、完整结果表、教学阈值子表、PCA、火山图、热图和 sessionInfo。
- 核验标准：比较方向正确；保留完整结果；图表参数清楚；能解释 999 是本轮阈值结果，不是固定生物学事实。

### 作业3：富集解释审阅

- 任务：使用给定的富集结果表和 AI 说明，标出输入基因、背景集、ID、数据库版本、FDR、基因集重叠和过度解释，并给出克制改写。
- 核验标准：区分 ORA/GSEA；不写通路激活、靶点或临床意义；缺证据处标 `需补证据`。
