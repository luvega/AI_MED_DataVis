...
AI 任务说明书把一次协作写成可执行、可核验、可验收的任务。它至少包含目标、上下文、约束、验证和输出。
...
| 第2章 | WorkBuddy 工作空间、默认权限、任务上下文、产物和变更核验 |
| 第3章 | AI 任务说明书、人工核验方法、`ai_log.md` 和版本留痕 |
| 第4章 | 按说明书亲手运行最小代码，解释输入、处理、输出和报错 |

在基础练习中，AI 可解释概念和报错、逐行说明已有短代码，并生成最小检查片段。完整脚本、清洗规则和结论仍由学习者完成并核验。
...
![图3-1 人在回路的 AI 协作与复核闭环](assets/chapter-3-human-ai-review-loop-imagegen.png)

*图3-1　闭环中的四个环节是：1，人定义问题、材料和证据边界；2，AI 提供代码或组织草稿；3，在授权工作空间内运行并保留检查点；4，人核对输出、来源和记录，再决定是否修订任务。生成式 AI 不承担最终科学判断。本图由 ImageGen 生成，准确关系以相邻流程图和正文为准，使用时仍需人工核对。*

任务从说明书开始。AI 产生输出后，学生按照预设要求完成人工核验，再决定是否验收，并把任务、修改和未解决事项写入协作记录。医药数据解释还要分开记录观察结果、方法来源、允许解释、替代解释和仍需验证内容。

## 3.1 从提示词到任务说明书

“帮我看看这个医药数据怎么处理”只表达了求助意图，没有定义可执行任务。AI 无法据此判断数据来源、字段含义、允许操作和交付物，也不知道遇到哪些缺口时应该停止。发出请求的人同样缺少核验依据，因为任何看似完整的回答都可能被解释为完成了任务。

任务说明书把模糊意图转换为一组可检查的约定。它先确定要解决的问题和输入材料，再限定允许与禁止的动作，写明通过标准和交付形式。这样，AI 的建议可以与原始要求逐项比较；即使任务失败，也能判断失败来自信息缺失、执行错误还是结果不符合标准。
...
目标、上下文、约束、验证和输出共同定义任务。目标说明要解决什么问题；上下文提供完成任务所需的文件与事实；约束限定可以怎样处理；验证预先规定检查方法、通过标准和停止条件；输出说明最终要交付什么。缺少任一项，其他字段的含义都可能随之改变。

例如，输出从“给出建议”改为“修改工作空间中的文件”后，约束必须补充可写路径，验证也要增加文件变更检查。输入数据从教学模拟表换成受限临床资料后，上下文和权限边界都要重写。五项骨架构成一组彼此校验的任务条件，填写一项时需要同步检查其余栏目。

任务信息齐全不等于必须使用五列表格。短任务可以写成五段文字，复杂任务可以拆出输入、参数、允许操作、禁止操作、输出格式和人工验收。拆分后的字段仍应能映射回五项骨架，使执行者和核验者看到同一套标准。
...
| 验收要求 | 需要提交什么、如何检查 | “越详细越好”这类空泛要求 |

文件任务的上下文还要写明 WorkBuddy 工作空间、默认可读写范围、需要人工确认的操作和结果保存位置。权限没有说明时，AI 应停在只读建议。
...
| 权限边界 | 写明工作空间、可读路径、可写路径、命令权限和待确认操作 |
...
验证要求、人工核验和验收按时间顺序发生。复核位于验收之后，可由同伴或独立流程再次检查。
...
学生把第14章扩展版字段映射回五项骨架，再为本章模拟表填写任务说明书。说明书要写清工作空间、默认权限、待确认操作、验证要求和结果保存位置。

## 3.3 AI 辅助代码生成与人工核验

基础练习把 AI 代码协作限制在解释已有短代码、生成用于核对字段或输出的最小片段，以及根据真实报错提出候选修改。范围较小，读者才能把输入、处理和输出逐行对应起来，并判断建议是否改变了原来的分析口径。

核验从阅读代码开始。先找出代码假设的路径、列名、类型和分组，再用最小输入运行；输出产生后，把行数、缺失值、字段和汇总口径与手工检查对照。报错消失只说明程序通过了当前执行点，不能证明数据含义、统计方法或医学解释已经正确。

分析脚本、清洗决策、统计方法和医学解释仍需学习者完成并核验。AI 可以提出候选方案，但采用哪一项以及为什么采用，必须留在运行记录和协作记录中。
...
`ai_log.md` 保存一次 AI 协作中影响结果的关键信息。它应说明任务怎样定义、AI 提出了什么、哪些建议被采纳或拒绝、代码是否实际运行，以及哪些问题仍未解决。聊天记录可以提供来源线索，却不能单独证明某段代码已经执行或某个文件已经按要求修改。

版本留痕补上文件层面的证据。修改前后对照或 Git diff 显示哪些内容发生变化，运行记录说明这些变化怎样产生输出，`ai_log.md` 则记录人为什么接受或撤销改动。三类记录指向同一份最终产物时，复核者才能从结论回到实际操作。

`ai_log.md` 至少包含七项：任务、原始提示词、AI 输出摘要、人工核验、修改记录、最终结果和反思。涉及文件修改或命令执行时，增加“工作空间与权限”，说明允许读写哪些路径、是否允许运行命令、哪些操作需要人工确认。会话记录只是来源之一，还要保存代码、真实输出、人工修改、文件变更和未解决问题。

| 字段 | 记录什么 | 不合格写法 |
| --- | --- | --- |
| 任务 | 这次让 AI 做什么 | 只写“问了 AI” |
| 工作空间与权限 | 工作空间、可读路径、可写路径、命令权限、待确认操作 | 没有说明 AI 能否修改文件 |
...
| 工作空间与权限 | 只审阅引用的代码；不修改文件，不运行外部命令 |
...
这份记录区分了 AI 输出和人工判断，也为代码运行状态和医学解释边界提供复核依据。
...
学术诚信要求提交者能够说明材料从哪里来、AI 参与了哪些环节、哪些内容经过实际运行或来源回查，并对最终文本、代码和图表负责。数据、文献、P 值、样本量和实验结果必须来自可回查材料或真实输出，不能由 AI 补造。披露 AI 使用只是起点，来源与结果仍要分别核验。

数据伦理把这项责任延伸到任务开始之前。医药数据可能包含直接身份信息、精确日期、患者编号或能够组合识别个人的字段。是否可以上传或交给外部服务，需要依据课程、机构和数据提供方的授权判断。权限不清时，应停止发送原始记录，改用字段说明、模拟数据或本地代码完成必要检查。

隐私、代码正确性和科学解释属于不同检查层。材料获准使用，不代表分析方法合适；程序正常运行，也不代表输出可以写成疗效、机制或临床建议。后面的案例分别处理这三层责任，并要求每一层都有对应的记录和停止条件。
...
后续单细胞章节可能使用参考模型给细胞生成候选标签。不能再让同一自动系统根据自己的标签判断“标注准确”。可接受的复核依据包括人工检查、独立校准样本、marker 组合、独立参考资源和供者层面的结果复核。
...
提交时还应附上 `ai_log.md`，据此核对 AI 做过什么、人工修改了什么、代码是否运行，以及验收结论由谁作出。
...
| 让 AI 自评 AI 标注 | 评价标准不独立 | 使用人工标注或独立确认样本 |
...
- 基础练习中，AI 只解释已有代码、生成最小检查片段或提出核验清单。
- AI 生成代码后，至少检查路径、列名、数据类型、缺失值、统计口径和输出结果。
- 医药解释区分数据观察、方法来源、允许解释和仍需验证内容。
- 证据不足处使用 `需补证据` 或 `需人工确认`。
- `ai_log.md` 包含七项 AI 协作记录；涉及文件修改或命令执行时，另记工作空间、权限、文件变更和待确认操作。
- 不把课堂模拟数据写成真实临床建议。
...
```mermaid
flowchart LR
  R["第3章"] --> N1["3.1 从提示词到任务说明书"]
  N1["3.1 从提示词到任务说明书"] --> N2["3.2 目标、上下文、约束、验证与输出"]
  N2["3.2 目标、上下文、约束、验证与输出"] --> N3["3.3 AI 辅助代码生成与人工核验"]
  N3["3.3 AI 辅助代码生成与人工核验"] --> N4["3.4 AI 协作记录与版本留痕"]
  N4["3.4 AI 协作记录与版本留痕"] --> N5["3.5 学术诚信、数据伦理与隐私保护"]
```
## 实验或作业

本章作业使用一段模糊提示词、一段有问题的 AI 代码初稿和一份模拟字段说明。提交任务说明书、核验清单和协作记录。
