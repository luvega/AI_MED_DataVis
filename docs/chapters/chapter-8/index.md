...
AI 可辅助生成局部检验代码、解释报错和整理结果表。分组独立性、检验前提、重复结构和医学边界必须人工核验。
...
![图8-1 从样本观察到有边界统计表述](assets/chapter-8-statistical-evidence-boundary-imagegen.png)

*图8-1　五个环节依次表示：1，从目标总体获得样本；2，描述样本和分布；3，表达估计的不确定性；4，并列考虑效应大小与统计证据；5，由人写出包含限制和替代解释的结论。本图由 ImageGen 生成，不对应真实研究结果，也不表示统计显著即可推出临床意义、机制或因果关系。准确概念以正文为准，使用时仍需人工核对。*
...
抽样误差指样本统计量因抽样而产生的波动。同一个总体中抽取不同样本，均值、中位数或比例通常不会完全相同。统计推断先承认这种波动，再估计当前样本结果对目标参数提供了多少信息。单次计算得到的均值因此只是一个点估计，还需要不确定性描述。

标准误（standard error, SE）描述估计量在重复抽样中的波动。以均值为例，样本内个体差异越大，均值的不确定性通常越大；样本量越大，均值的不确定性通常越小。常用计算式是 `SE = SD / sqrt(n)`。这里的 n 应对应统计独立单位，不能用同一受试者的重复测量或同一供体的大量细胞替代独立样本。

置信区间把点估计与不确定性放在一起。读区间时先确认估计对象和置信水平，再看上下限是否跨过研究关心的参考值。区间宽窄受数据变异、样本量和方法前提共同影响；它不表示大多数个体值位于其中，也不等于参数落入区间的简单概率陈述。
...
假设检验从预先写明的零假设开始。零假设（null hypothesis）通常表示没有差异、没有关联或参数等于某个参考值。检验方法再根据数据和模型前提，计算当前结果在这一假设下有多异常。若比较对象、方向或停止规则在看过结果后才改变，P 值的解释会失去原来的条件。

P 值（p-value）是在零假设和检验前提成立时，观察到当前或更极端结果的概率。它描述数据与假设模型之间的关系，不给出差异大小，也不直接给出零假设为真的概率。样本量、变异和检验选择都可能影响 P 值，因此不能脱离设计与方法单独阅读。

效应量回答差异或关联有多大，置信区间说明这一估计有多不确定，P 值提供假设检验下的统计证据。结果阅读应先看方向和大小，再看区间与 P 值，最后讨论这一大小在当前药学问题中是否具有实际意义。医学意义还需要结局定义、测量质量、研究设计和外部证据支持。
...
两组比较从研究设计开始。先确认结局变量类型，再判断两组来自不同独立单位还是同一单位的配对测量，随后查看缺失、分布、异常值和方差。表格中出现两列数字，只能说明数据被排成两组，不能自动确定检验方法。

独立与配对的区别会改变误差结构。若同一受试者治疗前后各有一次测量，分析应利用成对差值；若两组来自不同受试者，则需要独立组方法。把配对数据拆开会丢失个体内信息，把独立数据强行配对则会制造不存在的关系。

代码运行前写清比较方向、方法和纳入记录；运行后同时读取组别汇总、效应估计、置信区间、检验统计量和 P 值。若前提检查或异常观察改变了方法选择，应在记录中说明理由。任何检验输出都要回到原始分组和观察单位复核。
...
示例采用 Welch t 检验，因为它不要求两组方差相等，并据此估计自由度。真实任务仍应结合研究设计、分布、异常值和样本量选择方法。
...
当比较从两组扩展到三组及以上，或从一个指标扩展到多个指标时，发生偶然小 P 值的机会随之增加。若把许多检验分别执行，再只挑出 `P < 0.05` 的结果，报告中的证据强度会被高估。比较集合和校正策略应在查看显著结果前确定。

多组比较可以先用整体检验判断是否存在组间差异。连续变量多组均值比较常见入门方法是 ANOVA；如果前提不合适，可以考虑 Kruskal-Wallis 等非参数方法。整体检验并不指出哪些组不同，后续两两比较仍要说明事后比较范围和校正方式。

高维数据会把这一问题放大。差异表达结果中的 adjusted P value 或 `padj` 反映多重检验校正后的统计证据，但筛选阈值仍需与效应量、表达水平和研究目的共同解释。通过阈值的条目是后续核验候选，不能直接写成已经证实的机制。
...
统计结果应按证据层级书写。观察层先说明图表或汇总表中直接看到的方向和分布；推断层报告方法、独立样本量、效应量、置信区间和 P 值；医学解释层再把结果放回研究设计、结局意义和外部证据。层级分开，读者才能判断哪一句来自当前数据，哪一句还需要其他材料。

统计显著性不能修复设计缺陷。分组与批次混杂、选择性缺失、测量误差或伪重复都可能使精确的 P 值对应错误问题。医学解释还要考虑效应大小是否达到实际关心的程度、结果能否推广到目标人群，以及替代解释是否仍然存在。
