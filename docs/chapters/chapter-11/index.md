...
AI 可辅助生成较长的矩阵检查、PCA、聚类和热图代码。任务开始前应给出矩阵方向、元数据（metadata）、分组、批次、标准化、距离、输出和复核要求。
...
三类案例承担不同任务，数值和结论不能互相拼接。

| 案例 | 用途 | 证据状态 | 不可外推 |
| --- | --- | --- | --- |
| 8×6 微型矩阵 | 逐步解释矩阵方向、缩放、距离、PCA 和聚类 | 教学模拟 | 不代表真实高维生物数据 |
| NCI60 | 检查真实高维对象与跨语言结果 | 已在本机复跑；观察单位为细胞系 | 不作为患者分型或疾病机制证据 |
| airway top500 | 连接 RNA-seq 贯穿案例，检查样本结构与参数敏感性 | 课程复现数据 | top500 与 `rlog` 设置不是通用最优规则 |
...
高维数据把许多特征放在相对较少的样本上。此时，分析的首要对象仍是“样本 × 特征”矩阵及其元数据，而不是某个降维或聚类函数。矩阵的方向、数值含义、样本 ID 和分组注释一旦错位，后续方法可以正常运行，却会把标签贴到错误对象上。

进入计算前先写出 n 和 p，确认行列分别代表谁，检查特征与样本名称是否唯一，并把矩阵列与 metadata 行逐一对齐。高维只描述结构，不保证信息充分。特征越多，噪声、冗余、参数依赖和多重比较问题通常越需要显式检查。
...
高维分析首先要确认输入对象的含义。函数正常运行时，样本 ID 错配仍会把分组贴到错误样本上。应按主键匹配矩阵列名与元数据，再检查缺失和重复。
...
PCA、聚类和热图都从一个数值矩阵出发，但它们看到的样本关系会受预处理和距离定义影响。log 转换改变大值与小值的相对尺度，中心化移动特征的基准，按标准差缩放则改变各特征对距离和方差的贡献。处理步骤相同，不代表适用于所有数据含义。

每次转换都要保存输入矩阵、参数和输出矩阵，并抽查处理前后的范围、缺失和零值。距离计算前还要说明比较的是样本还是特征，以及使用何种尺度。若一个高变异特征主导全部距离，应判断这是研究对象的真实主轴，还是未处理的量纲或批次效应。
...
标准化会改变特征尺度，也可能放大低变异噪声。分析记录应说明缩放对象、选择理由及其对 PCA 和距离的影响。NCI60 案例保留缩放与不缩放的比较，不预设哪一种结果正确。
...
主成分分析把高维矩阵投影到少数新的坐标轴。输入矩阵决定可被概括的变化，预处理决定各特征对方差的贡献。PCA 不读取分组标签，因此图中出现的组间分离只能说明当前矩阵和参数下的主要变化与标签相对应，不能单独构成显著性或因果证据。

结果阅读需要同时看 scores、loadings 和解释比例。scores 给出样本在新坐标上的位置，loadings 说明原始特征对坐标轴的贡献，解释比例表示每个主成分概括了多少矩阵方差。只看散点颜色会忽略坐标来源，只看载荷排名又会丢失样本层结构。
...
聚类根据距离或相似性组织样本或特征，其输出依赖输入矩阵、距离度量、连接方法、簇数和随机初始化。分析者在运行前应说明聚类对象与参数，运行后再把簇与 metadata 交叉核对。元数据用于解释输出，不应悄然进入无监督输入。

聚类结果适合发现需要继续检查的结构。不同参数下稳定出现的样本邻近关系可以作为描述性证据，容易改变的分支则提示结果依赖选择。簇编号只是标签，不能自动对应疾病亚型、严重程度或机制类别。
...
热图把矩阵数值映射为颜色，并常用聚类重新排列行列。读图时要依次确认行对象、列对象、颜色尺度、是否按行或按列缩放、聚类方法和注释条来源。缺少其中任一信息，同一块颜色都可能被解释成原始高表达、相对高表达或仅仅是缩放后的局部差异。

生成热图前先固定展示矩阵与特征选择规则，生成后核对样本顺序、标签对齐和色标范围。注释条来自 metadata，不是热图算法产生的证据。色块模式可以帮助检查样本和候选特征，却不能单独证明分组差异或生物学机制。
...
真实矩阵可能有数千行，不能把所有标签堆进一张热图。筛选规则应在作图前确定，例如高变特征、给定特征或预先登记的功能集合。不能看完图后只保留支持预期解释的基因。
...
参数敏感性复核比较同一数据在合理分析选择下是否给出相近结构。airway 课程复现使用前 500 个高变基因和 `rlog(blind = FALSE)` 形成展示矩阵，这一设置只服务当前教学复现。替换为 `blind = TRUE` 或另一组特征后，应重新比较样本关系、解释比例和聚类结构，并记录哪些观察保持、哪些发生变化。

复核不是从多个结果中挑选最符合预期的一版。参数变化应有方法依据，所有版本使用同一比较问题和核验表。稳定部分可作为对当前选择不敏感的描述，变化部分则限定结论的适用条件；两者都需要保留，而不能只展示最整齐的图。
...
这组结果说明复现记录必须包括 Python/R 版本、包版本、随机种子和初始化次数。学生应保留实际输出，不能挑选“最接近教材”的一版。若更换环境，应重新保存成员分配和交叉表。
...
使用给定的 airway 前 500 个高变基因 rlog 矩阵和样本元数据，比较两种距离和两种 linkage，并说明哪些样本关系稳定；如另用模拟数据，须在图题、代码和报告中标明。
