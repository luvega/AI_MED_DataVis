...
教师提供一个样本×特征矩阵和元数据。学生比较两种标准化设置、两种距离和两种 linkage，并说明哪些样本关系稳定。若正式课堂数据尚未指定，使用模拟数据并在图题、代码和报告中标明。
...
| 位置 | 当前缺口 | 正文处理 |
| --- | --- | --- |
| 正式课堂数据集 | 尚未指定统一矩阵、元数据和数据字典 | 使用教学模拟数据并明确标注 |
| 课堂软件环境 | Python/R 包版本尚未固定 | 本轮运行记录保存当前版本，正式开课前再锁定 |
| 特征筛选规则 | 全部特征、高变特征或教师指定集合尚未确定 | 作业前由教师写入任务说明书 |
