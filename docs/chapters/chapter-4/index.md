...
| 误区 | 为什么错 | 如何纠正 |
| --- | --- | --- |
| 让 AI 一次生成完整代码 | 跳过原生语感培养 | 先手写 5 到 10 行最小代码 |
...
本章把模拟小表替换为统一教学数据包中的两个真实对象：`sample_metadata.csv`是8行样本数据框，`raw_counts.csv`是以基因为行、样本为计数列的矩阵型表。学生先打印对象结构，再进行索引和转换。

| 对象 | Python检查 | R检查 | 必须口头说明 |
| --- | --- | --- | --- |
| metadata | `shape`、`columns`、`dtypes` | `dim()`、`names()`、`str()` | 一行是一个bulk样本 |
| counts | ID列与8个样本列 | `rownames`与矩阵列名 | 行为基因、列为样本 |
| treatment | 唯一值与频数 | 因子水平与`table()` | `trt`和`untrt`的含义来自数据字典 |
| cell line | 唯一值与配对检查 | 交叉表 | 4个细胞系各有两种处理 |

一个可运行但错误的示例是：AI在Python中选取第1行后，将对应R代码写成`x[0, ]`。R返回的对象可能为空，学生要用最小对象手算并比较输出。另一个错误是无说明地转置counts；转置不是格式修复，而是改变观察对象。所有修正写入错误日志。
